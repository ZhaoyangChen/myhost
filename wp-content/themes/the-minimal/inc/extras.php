...
function the_minimal_social_cb(){
    ...
    if( $facebook || $twitter || $instagram || $google_plus || $pinterest || $linkedin || $youtube || $vimeo ){
    ?>
...
function the_minimal_footer_credit(){
    ...
    $text .= esc_html__( 'Theme by: ', 'the-minimal' );
    $text .= '<a href="' . esc_url( 'http://raratheme.com/' ) .'" rel="author">' . esc_html__( 'Rara Theme', 'the-minimal' ) . '</a> &middot; ';
    $text .= sprintf( esc_html__( 'Powered by: %s', 'the-minimal' ), '<a href="'. esc_url( __( 'https://wordpress.org/', 'the-minimal' ) ) .'">WordPress</a>' );
    $text .= '</p></div>';
